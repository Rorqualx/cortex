...
export function createTelegramInboundProcessing({
  ...
  const processInboundMessage = async (
    ...
    // Carry the carrying message id so debounce-merged turns hydrate media
    // against the actual source message instead of the merged current id.
    const inboundSourceMessageId =
      typeof msg.message_id === "number" ? { sourceMessageId: String(msg.message_id) } : {};
    const allMedia = nativeMedia
      ? [
          media
            ...
            : { kind: nativeMedia.kind, ...inboundSourceMessageId, unavailable },
        ]
      : [];

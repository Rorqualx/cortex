...
const sharedTelegramConfigUiHints = createChannelConfigUiHints({
...
export const telegramChannelConfigUiHints = {
  ...
  "dms.*.historyLimit": observedGroupHistoryHint,
  "accounts.*.dms.*.historyLimit": observedGroupHistoryHint,
  "": {
    label: "Telegram",
  ...
  ...sharedTelegramConfigUiHints,
  // Later keys replace whole hint objects, so merge the shared dmPolicy hint to
  // keep its label/help while restoring the primary-field ordering above.
  dmPolicy: { ...sharedTelegramConfigUiHints.dmPolicy, order: -70 },
  allowFrom: {
    order: -60,
  },
  groups: {

 ...
 */
export function classifyContentType(messages: ReadonlyArray<{ role?: string; content?: string }>): ClassificationResult {
  ...
  const hasCodeKeywords = /\b(function|const|class|import|export|def |async |await |return |if \(|for \(|=>)\b/.test(allText);
  ...
  const hasExecMarkers = /\b(exit code|stdout|stderr|command not found|✓|✗|PASS|FAIL)\b/i.test(allText);

  // Prose detection: natural language sentences
  const sentenceCount = (allText.match(/[.!?]\s/g) ?? []).length;
  const avgWordLength = allText.split(/\s+/).filter(Boolean).reduce((sum, w) => sum + w.length, 0) / Math.max(allText.split(/\s+/).filter(Boolean).length, 1);
  ...
  if (codeFenceCount >= 1) { codeScore += 2; signals.push(`code-fence:${codeFenceCount}`); }
  if (hasCodeKeywords) { codeScore += 1; signals.push('code-keywords'); }
  if (hasShellCommands) { codeScore += 1; signals.push('shell-commands'); }

  if (jsonBraceDensity > 0.02) { jsonScore += 1; signals.push(`brace-density:${jsonBraceDensity.toFixed(3)}`); }
  if (hasJsonKeys) { jsonScore += 2; signals.push('json-keys'); }

  if (hasToolRole) { toolScore += 2; signals.push('tool-role'); }
  if (hasExecMarkers) { toolScore += 1; signals.push('exec-markers'); }

  if (sentenceCount > 3 && avgWordLength > 3 && avgWordLength < 8) { proseScore += 2; signals.push(`prose:sentences=${sentenceCount}`); }
  if (codeScore === 0 && jsonScore === 0 && toolScore === 0) { proseScore += 1; }
  ...
  if (scores.length >= 2 && scores[0].score > 0 && scores[1].score > 0 && scores[0].score - scores[1].score <= 1) {
    return { type: 'mixed', confidence: 0.6, signals };
  }

  const winner = scores[0];
  return { type: winner.type, confidence: winner.score / (codeScore + jsonScore + proseScore + toolScore), signals };
}

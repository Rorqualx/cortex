...
# branch (main) in the MAIN tree, safely, as the deploy step. Relies on
...
if [ "$cur" != "main" ]; then
  echo "MERGE-ABORT: main is on '$cur', expected main"
  exit 2
fi
...
errlog="$(mktemp)"
# Prefer fast-forward (cron-work = main + new commits).

...
promote_release() {
  ...
  # APFS clonefile (cp -cR) is an instant copy-on-write copy; fall back to a plain copy on
  # a non-APFS volume. Each release is independent, so pruning one never affects another.
  if ! cp -cR "$root/dist" "$rel" 2>/dev/null; then
    rm -rf "$rel" 2>/dev/null || true
    cp -R "$root/dist" "$rel" || { echo "REFUSE-PROMOTE: could not clone dist into $rel" >&2; return 1; }
  fi

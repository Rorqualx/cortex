...
# Guard the expansion: under `set -u`, "${restage_files[@]}" on an empty array
# aborts with "unbound variable" on Bash 3.2 (macOS default) — reachable when
# every staged file is gitignored (e.g. a force-added ignored path).
format_files=()
...
if [ "${#format_files[@]}" -gt 0 ]; then
  # `--` terminates options so a filename beginning with `-` can't be read as an oxfmt flag.

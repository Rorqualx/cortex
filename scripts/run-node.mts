...
const computeBuildRequirement = (deps: RunNodeRequirementDeps): BuildRequirement => {
...
const computeRuntimePostBuildRequirement = (
...
// --- Managed-gateway build-suicide suppression (fork) ---
// run-node runs BEFORE any build and cannot import the compiled supervisor
// marker helper, so the marker contract is duplicated here. Conceptually mirrors
// hasOpenClawGatewayServiceMarker() in src/infra/supervisor-markers.ts.
export const GATEWAY_SERVICE_MARKER_ENV_KEYS = [
  "OPENCLAW_SERVICE_MARKER",
  "OPENCLAW_SERVICE_KIND",
] as const;

export const isManagedGatewayServiceEnv = (env: NodeJS.ProcessEnv): boolean =>
...
export const stripGatewayServiceMarkers = (env: NodeJS.ProcessEnv): NodeJS.ProcessEnv => {
  const stripped = { ...env };
...
// STALE-artifact reasons only: inside the managed gateway these rewrite the LIVE
// dist (or runtime-postbuild stamp) mid-flight and the launchd rebuild-restart
// watcher then bounces the gateway, killing the in-flight cron job ("interrupted
// by gateway restart", no deploy). MISSING-artifact reasons are excluded so a
// fresh/partial worktree still self-heals instead of crashing at runtime.
export const STALE_REBUILD_REASONS = new Set<BuildRequirement["reason"]>([
  "config_newer",
...
export const STALE_RUNTIME_POSTBUILD_REASONS = new Set<RuntimePostBuildRequirement["reason"]>([
  "build_stamp_newer",
...
/** Decides whether source changes require a new dev build (suppressed in the managed gateway). */
export const resolveBuildRequirement = (deps: RunNodeRequirementDeps): BuildRequirement => {
  const requirement = computeBuildRequirement(deps);
  if (
    requirement.shouldBuild &&
    STALE_REBUILD_REASONS.has(requirement.reason) &&
    isManagedGatewayServiceEnv(deps.env)
  ) {
    return { shouldBuild: false, reason: "auto_build_suppressed_in_service" };
  }
  return requirement;
};
...
export const resolveRuntimePostBuildRequirement = (
  ...
  const requirement = computeRuntimePostBuildRequirement(deps);
  if (
    requirement.shouldSync &&
    STALE_RUNTIME_POSTBUILD_REASONS.has(requirement.reason) &&
    isManagedGatewayServiceEnv(deps.env)
  ) {
    return { shouldSync: false, reason: "auto_build_suppressed_in_service" };
  }
  return requirement;
};
...
export const BUILD_REASON_LABELS = {
  ...
  auto_build_suppressed_in_service: "auto build suppressed inside managed gateway service",
  clean: "clean",
};

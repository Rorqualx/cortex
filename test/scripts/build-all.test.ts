...
describe("resolveBuildAllSteps", () => {
  ...
  it.each([
    ...
    async ({ env, cgroupGiB, heapMb, nodeOptions, warns }) => {
      ...
      // Fork: the SDK declaration writer is pinned to the 8192MB floor (huey
      // proof OOM at 4068MB, 2026-08-26); the tsdown plan's frozen budget still
      // governs every other step below.
      ...
      const expected = spawnSync(
        writer.command,
        ["--max-old-space-size=8192", ...probeArgs],
        probeOptions,
      );
      ...
      for (const invocation of invocations) {
        if (invocation === writer) {
          expect(invocation.options.env.NODE_OPTIONS).toContain("--max-old-space-size=8192");
        } else {
  ...
  it("includes ui:build in the full and ciArtifacts profiles after runtime postbuild", () => {
    for (const profile of ["full", "package", "ciArtifacts"]) {
      ...
      // Fork deploy contract: in the `full` profile runtime-postbuild-stamp is
      // the rebuild-restart trigger and runs LAST; other profiles keep the
      // upstream mid-list stamp position.
      if (profile === "full") {
        expect(labels.indexOf("ui:build")).toBeLessThan(labels.indexOf("runtime-postbuild-stamp"));
      } else {
        expect(labels.indexOf("ui:build")).toBeGreaterThan(
          labels.indexOf("runtime-postbuild-stamp"),
        );
      }

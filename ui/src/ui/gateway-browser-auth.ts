...
import { peekStoredDeviceIdentityId } from "./device-identity.ts";
...
export function hasStoredGatewayAuth(params: {
  ...
  // Fork device-auth store is single-key (not gateway-URL scoped); gatewayUrl
  // stays in the params contract for callers but does not key the lookup.

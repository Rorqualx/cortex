...
import type { ToolsGitHubStatusResult } from "../types.ts";
import type { GitHubIdentityController } from "./github-identity-controller.ts";
import { formatUiExternalText } from "./github-identity-format.ts";
import {
  renderSettingsRow,
...
// The fork has no shared copy-button component; this local handler copies the device
// code and briefly reflects the outcome on the button's visible label, then restores it.
async function handleGithubDeviceCopy(event: Event, text: string, idleLabel: string) {
  const button = event.currentTarget;
  if (!(button instanceof HTMLButtonElement) || button.dataset.copying === "1") {
    return;
  }
  ...
  label.textContent = copied ? t("common.copied") : t("common.copyFailed");
  window.setTimeout(() => {
      ...
      <div class="settings-row settings-row--actions">
        <div class="settings-row__control">
          ...
          <button
            ...
            @click=${(event: Event) =>
              void handleGithubDeviceCopy(event, authorization.userCode, copyLabel)}
          >

    ...
    ${renderSettingsSection(
      ...
      entries.length === 0
        ...
        : entries.map((entry) => {
            ...
            const defaultAccountId = params.snapshot?.channelDefaultAccountId?.[entry.id] ?? null;
            ...
            // Fork delta: keep per-account detail cards (default badge, status,
            // last-activity facts) under the upstream summary row.
            return html`
              ${row}
              ${entry.accounts.length > 0
                ? html`<div class="channel-accounts">
                    ${entry.accounts.map((account) =>
                      renderChannelAccount(account, account.accountId === defaultAccountId),
                    )}
                  </div>`
                : nothing}
            `;

...
import type { RunGitHubExternalMutation } from "./github-identity-controller-shared.ts";
...
function createController(behavior: { beforeDispatch?: () => void; refreshError?: string } = {}) {
  ...
  const runExternalMutation: RunGitHubExternalMutation = async (task, mutationOptions = {}) => {
...
describe("GitHubIdentityController", () => {
  ...
  // NOTE: upstream's "flushes a pending config draft before $mode" it.each block is
  // intentionally not ported. It drove a real fork runtime-config capability through
  // its `runExternalMutation` write-coordinator, which the fork's config capability
  // does not implement. Every other test here exercises the controller against a mock
  // host, so the controller's own behavior stays fully covered.
  ...
  it("invalidates selected System status when an override keeps the effective identity stable", async () => {
    ...
    const config = (systemProfileId: string) => ({
      ...
      agents: {
        list: [
          { id: "main", tools: { github: { profileId: "ghp_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" } } },
        ],
      },
    });
  ...
  it("waits for each authoritative poll deadline and adopts successful OAuth status", async () => {
    ...
    const client = { request } as unknown as GatewayBrowserClient;
    const runExternalMutation: RunGitHubExternalMutation = async (task) => ({
      ok: true,
      value: await task(client),
      refresh: { ok: true },
    });

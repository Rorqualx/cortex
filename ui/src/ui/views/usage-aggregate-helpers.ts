// Fork-local usage aggregate helpers.
//
// Upstream #131667 (b576bb41cf4 "fix(usage): retain reset history and unify
// accounting") removed mergeUsageLatency / mergeUsageDailyLatency /
// buildUsageAggregateTail from src/shared/usage-aggregates.ts in favor of the
// new accumulator API; upstream's Control UI no longer uses them. This fork
// still ships its own ui/src/ui usage-metrics view built on the old API, so
// the helpers live here — inside fork-owned ui/ — instead of being re-grafted
// into src/shared (which would recreate merge=ours drift on every sync).
// Bodies are verbatim from the pre-#131667 shared module.

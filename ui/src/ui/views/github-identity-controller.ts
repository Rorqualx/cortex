...
import { formatUiError } from "./github-identity-format.ts";

function readGitHubToolsConfig(tools: unknown): unknown {
  return isRecord(tools) ? tools.github : undefined;
}
...
export class GitHubIdentityController {
  ...
  sync(params: {
    ...
    // The fork's config-tools type does not model the `github` sub-object; read it
    // through a record coercion so the raw identity config stays typed as `unknown`.
    const values = {
      system: readGitHubToolsConfig(resolved?.globalTools),
      agent: readGitHubToolsConfig(resolved?.entry?.tools),
    };

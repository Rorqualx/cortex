...
import { renderSettingsSegmented } from "../views/settings-ui.ts";
import { t } from "../../i18n/index.ts";
import { isMockBoardEnabled, type BoardViewCallbacks } from "../board/provider.ts";
import type { BoardFace, BoardVisibleChatDock } from "../board/settings.ts";
import type { BoardTab } from "../board/types.ts";
import type { BoardViewSnapshot, BoardWidgetFrameUrl } from "../board/view-types.ts";
...
export async function ensureBoardViewElement(): Promise<boolean> {
  ...
  boardViewLoad ??= isMockBoardEnabled()
    ? import("../views/board-view-placeholder.ts")
    : import("../views/board/board-view.ts");
  await boardViewLoad;
  return true;
}

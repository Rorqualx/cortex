...
// Fork consumers (control-ui-performance, node tests) import the request
// timing shape from this module; alias the gateway-client type to keep them.
export type GatewayRequestTiming = GatewayProtocolRequestTiming;
...
export type GatewayConnectTiming = Omit<GatewayProtocolTiming<ConnectPlan>, "plan" | "detail"> & {
...
export class GatewayBrowserClient {
  ...
  private handleConnectHello(hello: GatewayHelloOk, plan: ConnectPlan) {
    ...
    if (hello?.auth?.deviceToken && plan.deviceIdentity) {
      // Fork device-auth store is single-key (not gateway-URL scoped); see
      // ui/src/ui/device-auth.ts before reintroducing upstream's gatewayUrl arg.

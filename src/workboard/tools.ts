...
function contextOwner(ctx: Record<string, unknown> | undefined): string {
  const record = ctx ?? {};
  return (
...
export function createWorkboardTools(
  initialStore?: WorkboardStore,
  context?: Record<string, unknown>,
): AnyAgentTool[] {
  const store = initialStore ?? WorkboardStore.openSqlite();
  const ownerId = contextOwner(context);

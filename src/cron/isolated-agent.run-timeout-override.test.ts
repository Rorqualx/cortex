...
describe("resolveCronRunTimeoutOverrideMs", () => {
  ...
  // explicit-timeout signal so the run budget and provider-timeout bounds stay
  // accurate. The LLM idle watchdog clamps this to its implicit cap regardless:
  // the job budget is not a per-call silence allowance; slow silent providers
  // opt out via `models.providers.<id>.timeoutSeconds`.

...
async function initSessionStateAttemptLocked(
  ...
  // Upstream's one-shot reconnect-resume signal suppresses configured idle/daily rollover.
  const reconnectResumeRequested =
    params.resumeRequestedSession === true && requestedCurrentSession;
  // Fork: Control-UI chat tabs are user-managed (explicit "New session" button), so the
  // daily/idle auto-reset must never rotate them out from under an open tab; this is a
  // deliberate divergence from upstream (which lets dashboard sessions roll over except on
  // reconnect-resume). Only channel/provider-owned sessions implicitly expire otherwise.
  ...
  const skipImplicitExpiry =
    ...
    (hasProviderOwnedSession(entry) && resetPolicy.configured !== true);

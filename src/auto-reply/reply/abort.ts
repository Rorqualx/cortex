...
export async function stopSubagentsForRequester(params: {
  ...
  for (const run of runs) {
    ...
    // Liveness, queue clearing and terminated-marking all live in
    // killControlledSubagentRun (src/agents/subagent-control.ts) — it owns the run
    // lifecycle. Aborting inline here duplicated that policy and could not see
    // cascade kills, so the counts under-reported.

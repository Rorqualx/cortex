...
function buildSubagentDelegationPreferenceSection(params: {
  ...
  return [
    ...
    '- Set `taskName` when you will need a stable handle later; keep it lowercase with underscores or hyphens. Set `label` to a short task title for UI lists, not a persona. Omit `context` for isolated children; set `context:"fork"` only when current transcript details matter.',
...
function buildMessagingSection(params: {
  ...
  // Telegram rich text is gated per channel/account capability; both the enabled
  // and disabled lines are surfaced so the agent never assumes the wrong mode.
  ...
  const subagentOrchestrationGuidance = hasSessionsSpawn
    ? hasSubagents
      ? `- Sub-agent orchestration → use \`sessions_spawn(...)\` to start delegated work; include a clear objective/output/write-scope/verification brief and \`taskName\` when a stable handle helps and \`label\` for a short UI task title; omit \`context\` for isolated children, set \`context:"fork"\` only when the child needs the current transcript; ${hasSessionsYield ? "use `sessions_yield` to wait for completion events; " : ""}use \`subagents(action=list)\` only for on-demand status/debugging visibility.`

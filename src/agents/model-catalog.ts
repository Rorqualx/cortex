...
// Fork: live-discovery overlay (auto-populate discovered models, hide deprecated).
...
export function loadManifestModelCatalog(params: {
  ...
  const rows = plan.rows.map((row) => {
    ...
    // Fork: carry deprecation-reassignment lineage so replaced models can be
    // remapped when a provider retires an id.
...
// Fork: overlay the live-discovery snapshot (auto-populate new models, hide
// deprecated) onto an assembled catalog. Best-effort: a state-DB issue leaves
// the catalog as-is.
...
export async function buildPreparedModelCatalogSnapshot(
  ...
  try {
    ...
    // Fork: overlay the live-discovery snapshot (auto-populate new models, hide
    // deprecated) onto the assembled catalog before snapshotting. Best-effort:
    // a state-DB issue leaves the catalog as-is. Deprecated-hidden entries stay

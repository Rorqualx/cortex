...
export async function acquireSessionWriteLock(params: {
  ...
  try {
    ...
    if (error instanceof OpenClawStateLeaseError && error.code === "OPENCLAW_STATE_LEASE_TIMEOUT") {
      // A lease still tracked in this process means a concurrent run *here* holds it.
      // That contention is legitimate progress, not a failure, so callers can show a
      // "still working" notice instead of a lock diagnostic (see
      // isSessionWriteLockBusyWithActiveRun). Cross-process owners stay anonymous.
      const heldInThisProcess = sessionWriteLeaseState.held.has(params.sessionFile);
      throw new SessionWriteLockTimeoutError({
        timeoutMs,
        owner: heldInThisProcess
          ? "another run in this OpenClaw process"
          : "another OpenClaw process",
        lockPath: leasePath(params.sessionFile),
        ...(heldInThisProcess ? { ownerPid: process.pid, ownerPidAlive: true } : {}),
      });
    }
    throw error;
  } finally {

...
/**
 * Core convenience for callers that run a single short-lived completion and do
 * not manage the acquisition lease themselves: acquire, release the internal
 * lease immediately, and hand back the prepared facts. Plugin consumers use the
 * host-tracked `prepareSimpleCompletionModelForAgent` in
 * `plugin-sdk/simple-completion-runtime.ts` instead; that variant keeps the
 * lease alive under the legacy plugin-SDK resource host.
 */
export async function prepareSimpleCompletionModelForAgent(
  ...
  // Release the internal lease immediately (upstream models it as AsyncDisposable),
  // then hand back the prepared facts without the disposer.
  const { [Symbol.asyncDispose]: dispose, ...prepared } = acquired;
  await dispose.call(acquired);
  return prepared;
}

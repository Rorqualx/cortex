...
/**
 * Truncates the transcript addressed by an explicit session file.
 *
 * Fork-owned entry point for the context-overflow recovery path in `run.ts`.
 * Upstream retired the raw-JSONL read/rewrite pair this used to call, so it now
 * delegates to the unified runtime target rather than keeping a second
 * file-backed transcript path alive.
 */
export async function truncateOversizedToolResultsInSession(params: {
  ...
  return await truncateOversizedToolResultsInActiveTarget({
    ...
    contextWindowTokens: params.contextWindowTokens,
    ...(params.maxCharsOverride !== undefined ? { maxCharsOverride: params.maxCharsOverride } : {}),
    ...(params.aggregateMaxCharsOverride !== undefined
      ? { aggregateMaxCharsOverride: params.aggregateMaxCharsOverride }
      : {}),
    ...(params.protectTrailingToolResults !== undefined
      ? { protectTrailingToolResults: params.protectTrailingToolResults }
      : {}),
  });
}

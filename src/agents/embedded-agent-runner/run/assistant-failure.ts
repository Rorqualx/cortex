...
export async function handleEmbeddedAssistantFailure(input: {
  ...
  const outcome = await handleAssistantFailover({
    ...
    // This run-loop.ts-only path (dead: run-orchestrator.ts has no live caller
    // today) has no rate-limit rotation budget of its own; fall through to the
    // normal rotate/fallback branch instead of a same-model retry.

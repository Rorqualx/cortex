...
export async function prepareEmbeddedAttemptSessionManager(input: {
  ...
  const sessionManager = guardSessionManager(
        ...
        : SessionManager.inMemory(input.effectiveCwd)),
    {
      ...
      // Producer for onAssistantMessagePersisted: the session-lock removal dropped the
      // fork's onMessagePersisted wire, orphaning the steered-follow-up reply gate.
      // Forward persisted assistant messages so a follow-up steered into an active run
      // is classified and broadcast out-of-band (see chat-send-agent-dispatch).
      onMessagePersisted: (message) => {
        if (message.role === "assistant") {
          void attempt.onAssistantMessagePersisted?.(message);
        }
      },
    },
  );

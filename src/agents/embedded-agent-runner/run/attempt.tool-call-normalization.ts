...
export function sanitizeReplayToolCallIdsForStream(params: {
  ...
  // Fork perf guard: the pairing repair is an O(N^2) full-transcript pass;
  // agent-core hands us an already-canonically-paired transcript on nearly
  // every tool-call continuation, so skip it unless a real pairing defect is
  // present. Upstream ordering is preserved: repair runs before id rewriting.
  const needsPairingRepair =
    params.repairToolUseResultPairing === true && !isToolUseResultPairingValid(params.messages);
  const paired = needsPairingRepair

...
import { applySessionEntryReplacements } from "../../config/sessions/session-accessor.lifecycle.js";
...
export async function recoverStore(params: {
  ...
  for (const { sessionKey, entry: loadedEntry } of entries.toSorted((a, b) =>
    ...
    // Fork self-heal: a webchat chat whose delivery routing drifted to a real
    ...
    if (isDriftedWebchatRouting(entry)) {
      ...
      await applySessionEntryReplacements({
        ...
        update: (entries) => {
          // Reset the freshly-loaded locked entry (not the unlocked snapshot)

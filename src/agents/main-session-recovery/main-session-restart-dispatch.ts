...
export async function resumeMainSession(params: {
  ...
  try {
    ...
    const agentParams: AgentRunRequest = {
      ...
      // Fork: the "[System]" continuation is a recovery instruction for the
      // model, not a user turn: persisting it renders a bogus "You" bubble in
      // the dashboard (the Control UI classifies bubbles purely by role).
      // Suppress persistence so the model still sees it this run while the
      // transcript records only the resumed assistant reply. Unlike the sibling
      // subagent-orphan-recovery we deliberately do NOT set
      // sessionEffects:"internal" — the main session must stay visible to
      // deliver.

...
/** Names apply_patch in cross-session write-guard errors (see file-write-guard). */
const APPLY_PATCH_MUTATION_OPTIONS = { toolName: "apply_patch" } as const;
  ...
  for (const hunk of parsed.hunks) {
    ...
    if (hunk.kind === "add") {
      ...
      await withFileMutationQueueKeyResolution(
        ...
        {
          ...APPLY_PATCH_MUTATION_OPTIONS,
          filePaths: targetResolution.then((target) => [target.resolved]),
        },
      );
      const target = await targetResolution;
    ...
    if (hunk.kind === "delete") {
      const targetResolution = resolvePatchPath(hunk.path, options, PATH_ALIAS_POLICIES.unlinkTarget);
      await withFileMutationQueueKeyResolution(
        targetResolution.then((target) => target.queueKey),
        async () => {
          const target = await targetResolution;
    ...
    await withFileMutationQueueKeysResolution(
      ...
      },
      {
        ...
        filePaths: Promise.all([
          targetResolution.then((target) => target.resolved),
          ...(moveTargetResolution
            ? [moveTargetResolution.then((moveTarget) => moveTarget.resolved)]
            : []),
        ]),
      },
    );

...
function toAgentToolResult(params: {
  ...
  const normalizedContent: AgentToolResult<unknown>["content"] = hasStructured
    ? [
        ...
        // Structured JSON mirrors text only; the original image/audio/resource
        // blocks carry payload it cannot represent, so they ride along fenced.
        // Filter the RAW blocks: resource_link/resource/audio only become text
        // once converted, so filtering the converted list would drop them.
        ...(Array.isArray(params.result.content) ? params.result.content : [])

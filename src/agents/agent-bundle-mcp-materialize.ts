    ...
    const tools = buildBundleMcpToolsFromCatalog({
      ...
      createExecute: (tool) => (toolCallId: string, input: unknown, signal?: AbortSignal) =>
        runWithSessionMcpRequestSignal(signal, async () => {
          ...
          // Fork: MCP servers are untrusted. Project once through the shared boundary
          // helper, then fence every model-facing text block and sanitize the structured
          // details channel so server output cannot forge tool calls or corrupt
          // downstream structured merging.
          const projected = projectMcpCallToolResult(result, {

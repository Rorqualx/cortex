...
async function persistUserTurnTranscriptToFile(params: {
  ...
  let appended = turn.messages[0] as
    ...
    | undefined;
  if (appended && !appended.anchor && appended.message.role === "user" && target.sessionId) {
    const anchorScope = {
  ...
  const recorder: UserTurnTranscriptRecorder = {
    ...
    stageApproved: (options) => {
      staging ??= (async () => {
        ...
        // Fork: explicit-file targets have no session store to hold durable
        // pending input; their turns persist straight to the transcript file
        // via the persistUserTurnTranscriptToFile branch instead.
        if (isUserTurnTranscriptFileTarget(target)) {
          return false;
        }

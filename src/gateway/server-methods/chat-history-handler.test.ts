    ...
    async (method) => {
      await withOpenClawTestState({ scenario: "minimal" }, async () => {
        ...
        try {
          ...
          // Fork: the wire schema has no cursor catch-up param (the fork UI
          // pages with offset/pendingBefore); a repeat call re-requests the
          // same receipts for the same inputRunIds.
          const repeat = await call({ inputRunIds });
          expect(repeat.inputReceipts).toEqual(expected);

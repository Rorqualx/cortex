...
export function dispatchGatewayCronFinishedNotifications(params: {
  ...
  // The check above only covers a completionDestination webhook. These two cover the
  // other ways a configured completion silently reaches nobody: an announce with no
  // channel/to, and a top-level webhook delivery whose `to` never resolved a target.
  ...
  // resolveCronCompletionWebhook only resolves a completionDestination webhook
  ...
  if (completionWebhookUrl === undefined && params.job?.delivery?.mode === "webhook") {
    params.logger.warn(
      {
        jobId: params.evt.jobId,
...
function dispatchCronCompletionAnnounce(params: {
  ...
  for (const target of announceTargets) {
    // Completion announces reuse upstream's admission-wrapped, timeout-bounded
    // cron announce sender (best-effort): keep the fork's success-summary
    // delivery, drop the failure semantics its retired sender carried. #126483
    // removed the old dispatchCronFailureDestinationNotifications path.
    void sendGatewayCronFailureAlert({
      deps: params.deps,
      logger: params.logger,
      resolveCronAgent: params.resolveCronAgent,
      job,
      ...
      params.logger.warn(
        ...
        "cron: completion announce failed",
      );
    });
  }
}

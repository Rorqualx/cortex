...
export function getSessionDefaults(
  ...
  // configured primary (agents.list[<id>].model.primary) and context cap instead
  // of the global agents.defaults, so the dashboard "Default (…)" label tracks the
  // active agent rather than drifting to a diverged global default (e.g. a quota
  // fallback that left agents.defaults pinned to a different model).
  const resolved = scopeAgentId
    ? resolveDefaultModelForAgent({
        cfg,
  ...
  const contextTokens =
    (scopeAgentId
      ? resolveAgentConfig(cfg, scopeAgentId)?.contextTokens
      : cfg.agents?.defaults?.contextTokens) ??
    lookupContextTokens(resolved.model, { allowAsyncLoad: false }) ??
    DEFAULT_CONTEXT_TOKENS;
  const agentId = normalizeAgentId(scopeAgentId ?? resolveDefaultAgentId(cfg));

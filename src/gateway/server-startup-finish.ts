          ...
          gatewayRuntimeServices.scheduleGatewayPostReadyMaintenance({
            ...
            applyMaintenance: async (maintenance) => {
              if (lifecycle.closePreludeStarted) {
                ...
                // Match clearGatewayMaintenanceHandles: the fork's workboardDispatch
                // interval is started unconditionally, so this shutdown-race path must
                // clear it too or it keeps spawning workboard subagents against a
                // gateway whose state is being torn down (and blocks event-loop quiesce).

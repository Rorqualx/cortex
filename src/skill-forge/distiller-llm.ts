...
function buildUserPrompt(candidate: Candidate): string {
  return [
    ...
    "",
    "Available tool schemas (you may reference these in the skill body):",
    "- read(path) — read file contents at a path",
    "- write(path, content) — write string content to a file, creating parent directories",
    "- edit(path, edits) — targeted text replacements in a file via oldText→newText pairs",
    "- exec(command) — run a shell command and capture stdout/stderr",
    "- apply_patch(input) — apply a multi-file patch with markers",
    "- memory_get(path) — read from the agent's memory store",
    "- memory_search(query) — semantic search across memory files",
    "- glob(pattern) — find files matching a glob pattern",
    "- grep(pattern, path?) — search file contents with a regex pattern",
    "- web_search(query) — search the web for information",
    "- web_fetch(url) — fetch and extract content from a web page",
    "",

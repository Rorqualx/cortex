...
// Fork: workboard legacy-DB migration runs alongside upstream's state-schema
// detection/repair (unified shared-DB workboard store).
...
export async function runLegacyStateMigrations(params: {
  ...
  // Fork: run upstream's foundational state-schema migration first (bail on
  // legacy-schema warnings), then the workboard legacy-DB migration.
...
export async function autoMigrateLegacyState(params: {
  ...
  // Fork: runs unconditionally (a no-op when the legacy workboard DB is absent)
  // so it still fires on the early-return paths below, where no other legacy
  // state exists.
  const workboardDb = await migrateLegacyWorkboardDb({ stateDir: detected.stateDir });

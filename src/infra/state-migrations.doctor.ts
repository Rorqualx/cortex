...
async function executeLegacyStateMigrations(
  ...
  // Fork: unconditional (a no-op when the legacy workboard DB is absent) so it
  // still fires on every early-return branch, where no other legacy state exists.
  const workboardDb: MigrationMessages = await migrateLegacyWorkboardDb({ stateDir });
